...
def my_run_with_flags(generator, keras_eventsequence_output, index):
  ...
  for i in range(FLAGS.num_outputs):
    # keras_eventsequence_output = [0,1,2,3,4,5,6,7,8]
    #generated_sequence = generator.generate(input_sequence, generator_options)
    generated_sequence = generator.numpy_to_midi(   input_sequence,
                                                    generator_options,
                                                    keras_eventsequence_output)


    print("generated_sequence:\n",generated_sequence)

    midi_filename = '%s_%s_%s.mid' % (date_and_time, str(i + 1).zfill(digits), str(index))
...
def main(unused_argv):
  ...
  for i in range(11):
      ...
      onehot_to_number = np.argmax(test_song, axis=1)

      numpy_event_sequence = list(onehot_to_number)

      my_run_with_flags(generator, numpy_event_sequence, i)
