...
exp_name = 'Embedding_method_Dense10_Infolen%d_LS%d_WinSize%d_BS%d_%dpart_epoch%d' % (infor_length, LayerSize, time_step, batch_size, how_much_part, epochs)
...
def on_epoch_end(epoch, logs):
    ...
    if((epoch+1)%40 == 0):
        ...
        start_index = random.randint(0, len(text) - time_step - infor_length)
        for diversity in [0.2, 0.5, 1.0, 1.2]:
            print_func('----- diversity:' + str(diversity))

            generated = ''
            sentence = text[start_index: start_index + time_step + infor_length]
            generated += sentence
