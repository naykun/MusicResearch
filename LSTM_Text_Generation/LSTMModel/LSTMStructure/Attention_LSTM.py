...
exp_name = 'Attention_LayerN%d_LS%d_WinSize%d_BS%d_%dpart_epoch%d' % (LSTM_Layer_Num, LayerSize, maxlen, batch_size, how_much_part, epochs)
log = '../res/' + exp_name + '.txt'
tb_log = '../TB_logdir/LSTM/RNNStructure_6_30/' + exp_name
max_acc_log = '../res/attention_max_acc.txt'
...
def model_attention_applied_after_lstm():
    inputs = Input(shape=(TIME_STEPS, maxlen,len(chars),))
    lstm_units = LayerSize
    lstm_out = LSTM(lstm_units, return_sequences=True)(inputs)
    attention_mul = attention_3d_block(lstm_out)
    attention_mul = Flatten()(attention_mul)
    output = Dense(len(chars), activation='sigmoid')(attention_mul)
