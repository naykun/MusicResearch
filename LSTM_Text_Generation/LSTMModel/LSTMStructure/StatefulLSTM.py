...
from keras.layers import Dense, Activation, LSTM, Input, Reshape, MaxPooling1D, Conv1D, Dropout, Bidirectional
...
exp_name = 'Embedding_method_Stateful_LSTM_Infolen%d_LS%d_WinSize%d_BS%d_%dpart_epoch%d' % (infor_length, LayerSize, time_step, batch_size, how_much_part, epochs)
...
range_for_state = x.shape[0] - x.shape[0] % batch_size
x = x[:range_for_state]
y = y[:range_for_state]
print(np.shape(x))
print(np.shape(y))
...
print('Build model...')
# 这部分返回一个张量
inputs = Input(batch_shape=(batch_size, time_step, len(chars)*infor_length,))
...
# xxx = K.permute_dimensions(inputs, (0,2,1))
# xxx = Bidirectional(LSTM(LayerSize, return_sequences=True, return_state=True, dropout=0))(inputs)

xxx = LSTM(LayerSize, return_sequences=False, stateful=True)(inputs)
