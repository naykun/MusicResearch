...
def get_complex_model(input_shape_melody, input_shape_accom,  output_shape):
    ...
    def get_melody_feature(main_melody):
        ...
        x = LocallyConnected1D(filters=16, kernel_size=16, padding='valid', activation='sigmoid',strides=1)(x)
        x = LocallyConnected1D(filters=32, kernel_size=8, padding='valid', activation='sigmoid', strides=1)(x)
        x = GlobalMaxPooling1D()(x)
        x = Dense(melody_feature_length, activation='sigmoid')(x)
        return x

    def get_current_melody_feature(before_accom, current_melody):
        print('before_accom', before_accom.shape)
...
def get_conv1d_model(input_shape, output_shape):
    ...
    xxx = Conv1D(filters=32, kernel_size=20, padding='same', activation='sigmoid',strides=1)(xxx)
    xxx = Conv1D(filters=64, kernel_size=10, padding='same', activation='sigmoid', strides=1)(xxx)
    # we use max pooling:
...
def get_conv1d_resnet(input_shape, output_shape):
    return resnet_v1(input_shape,num_classes=output_shape, depth=3*6+2)